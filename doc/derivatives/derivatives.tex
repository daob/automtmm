...
\section{Monte Carlo evaluation of the approach}
...
The standardized estimates are unbiased, the standard errors are very close to the monte carlo standard deviations 
of the standardized estimates. The coverage of the 95\% confidence confidence intervals using the Fisher $z$-transform
is close to 95\%, with the lowest observed coverage 95\%  and the highest coverage 97\%. The mean and median coverage
of the 95\% coverage intervals across the 24 standardized parameters are 94.7\% and 95.0\%. 

\begin{figure}[htb]
\centering
\subfigure[Reliability (left) and $z$ transform (right) distributions in group with only Primary education]{
	\includegraphics[width=.48\textwidth]{normplots/normality-yr1-gp1}
   \label{fig:subfig1}
 }
 \subfigure[Reliability (left) and $z$ transform (right) distributions in group with University education]{
	\includegraphics[width=.48\textwidth]{normplots/normality-yr1-gp6}
   \label{fig:subfig2}
 }	\caption{Distribution and normal Q-Q plots of the reliability coefficients and their Fisher $z$-transforms
 	 in 2007 in the lowest and highest educational group.}
\end{figure}
 9 out of 24 Shapiro-Wilks tests for the raw stdized estimates are adjusted-significant, none of the transformed ones are. So it seems the 
the normal approximation is better for the transformed. in exactly 2 thirds of the cases the fit is better for the transformed than for the
raw standardized values.

Note that the sample sizes in 
some of the groups are relatively small, with $n$ ranging from 256 to 799 observations (see first column of table \ref{tab:health-education}).
The approach taken here of providing asymptotic standard errors and confidence intervals for the 
standardized estimates works well in this example, in spite of the smaller sample sizes. 
