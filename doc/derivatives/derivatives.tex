...
From definition \ref{eq:beta_s}, 
\begin{multline}\label{eq:dvecbet}
\d\vec B^{(s)} = 
    (D_\n B' \kronprod I_p) \d \vec D_\n^{-1} + 
    (D_\n \kronprod D_\n^{-1}) \d\vec B + \\
    (I_q \kronprod D_\n^{-1} B) \d\vec D_\n.
\end{multline}
...
Applying standard operations on \ref{eq:dveclam} and rearranging terms, 
